...
test.describe('training stabilization', () => {
  ...
  test('unfinished-set confirmation preserves cancellation and saves only completed work', async ({ page }) => {
    ...
    const firstWeight = page.getByLabel('Weight, Phase 1 Bench Press, set 1, kg').first()
    const firstReps = page.getByLabel('Reps, Phase 1 Bench Press, set 1').first()
    await expect(firstWeight).toHaveValue('80')
    await firstWeight.fill('60')
    await firstReps.fill('8')
    await page.getByRole('button', { name: 'Add set' }).click()
    await page.getByLabel('Weight, Phase 1 Bench Press, set 2, kg').first().fill('60')
    await page.getByLabel('Reps, Phase 1 Bench Press, set 2').first().fill('8')
  ...
  test('kg and lbs presentation preserves completed-workout storage while editing', async ({ page, cleanup }) => {
    ...
    const detailTable = page.getByRole('table', { name: 'Sets: Bench Press' })
    await expect(detailTable.getByRole('columnheader', { name: 'Weight lbs' })).toBeVisible()
    await expect(detailTable.getByRole('cell', { name: '143.3' })).toBeVisible()
    const visibleEdit = page.locator('button:visible').filter({ hasText: /^Edit workout$/ }).first()
    await visibleEdit.click()
    const weightInput = page.getByRole('spinbutton', { name: 'Weight, Bench Press, set 1, lbs' })
    await expect(weightInput).toHaveValue('143.3')
    await page.locator('button:visible').filter({ hasText: /^Save$/ }).first().click()
    await expect(weightInput).toHaveCount(0)
    expect((await readLifecycleWorkout(sessionId))?.exercises?.[0]?.sets?.[0]?.weight).toBe(65)

    await page.locator('button:visible').filter({ hasText: /^Edit workout$/ }).first().click()
    const fractionalWeight = page.getByRole('spinbutton', { name: 'Weight, Bench Press, set 1, lbs' })
    ...
    await expect(page.getByRole('table', { name: 'Sets: Bench Press' }).getByRole('columnheader', { name: 'Weight kg' })).toBeVisible()
  })
})
