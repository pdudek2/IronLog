...
- [ ] **Step 1: Start Auth/Firestore emulators, development API and Vite on free local ports.**
- [ ] **Step 2: Create a fresh local audit account and complete onboarding through the UI.**
- [ ] **Step 3: Seed only the minimum deterministic documents needed for filled states using existing schemas/helpers.**
- [ ] **Step 4: Confirm every protected route loads in the same signed-in Browser session and that no production service is targeted.**
...
- [ ] **Step 1: Run the existing accessibility and contrast desktop/mobile gate against emulators.**
- [ ] **Step 2: Run the smallest route-specific checks needed for unverified shell, workout, history, template, exercise and Coach states.**
- [ ] **Step 3: Reproduce any diagnostic on unchanged current `main` before classifying it as a product defect.**
...
- [ ] **Step 1: Observe each public and onboarding surface, including focus and redirect outcomes.**
- [ ] **Step 2: Observe every protected surface in its primary loaded state.**
- [ ] **Step 3: Deepen the materially different empty, filled, expanded, destructive, disabled/locked, focus and offline/error states named in the ledger.**
- [ ] **Step 4: Measure horizontal overflow, essential text floor, touch geometry and fixed/sticky overlap at all four viewports.**
- [ ] **Step 5: Capture representative current-runtime screenshots and record Browser warnings/errors.**
- [ ] **Step 6: Reset temporary viewport overrides, close the created Browser tab and stop the local runtime.**
...
- [ ] **Step 1: Apply every category in the visual-audit rubric and record `PASS`, `POLISH`, `MATERIAL`, `BLOCK` or `N/A` with concrete evidence.**
- [ ] **Step 2: Write the prioritized audit: verdict, coverage, keep, findings, system pattern, next-pass brief, unknowns and tooling notes.**
- [ ] **Step 3: If no `BLOCK` or `MATERIAL` remains, mark Etap 5 complete; otherwise open the smallest named refinement slice and keep Etap 5 active.**
- [ ] **Step 4: Mark this plan `DONE`, append an execution receipt and commit only the 5C plan/report/evidence plus the roadmap update.**
