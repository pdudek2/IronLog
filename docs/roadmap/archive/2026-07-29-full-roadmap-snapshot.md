# IronLog — archiwalny snapshot roadmapy A–7

Status dokumentu: **ARCHIVED — tylko historia i traceability**
Stan przeglądu: **APPROVED — poprzedni program zakończony; po Agent Sanity Review z 2026-07-29 otwarto program korekcyjny 8A–8D oraz końcową bramkę 9**
Źródła: audyt techniczny, pierwszy audyt UI, Senior Design Review z 2026-07-14, pełny audyt runtime z 2026-07-20 oraz Agent Sanity Review z 2026-07-29
...
- lint przechodzi,
- build przechodzi,
- 472 testy jednostkowe oraz testy wsparcia przechodzą,
- 1 plik i 17 testów reguł Firestore przechodzi,
- ukierunkowana integracja zamknięcia i projekcji workoutu przechodzi: 2 pliki i 20 testów,
- historyczne izolowane bramki Auth+Firestore i krytycznych przepływów przeszły przed wydaniem Puls,
- po zmianach copy część E2E używa nieaktualnych nagłówków i nie stanowi obecnie wiarygodnej bramki; naprawę i ponowną walidację obejmuje Faza 8A,
- live `npm run test:e2e` nadal wymaga prywatnych `TEST_EMAIL` i `TEST_PASSWORD`.
...
| 15 | 8A — Higiena release i wiarygodność E2E | P1 | READY | Repo nie zawiera stanu logowania przeglądarki ani publicznych artefaktów review, a testy rozpoznają aktualne ekrany |
| 16 | 8B — Serializacja projekcji workoutu | P1 | DESIGN IN PROGRESS | Usuwanie i materializacja jednego workoutu nie mogą odtworzyć usuniętych danych |
| 17 | 8C — Integralność katalogu planów AI | P2 | DESIGN IN PROGRESS | Plan nie mapuje ćwiczeń po niejednoznacznej nazwie i jawnie obsługuje niepełny katalog |
| 18 | 8D — Kontrakt repo i cleanup | P2 | READY | Instrukcje odpowiadają aktualnej architekturze, a martwy scaffolding znika |
| 19 | 9 — Korekcyjna bramka wydania | P1 | BLOCKED | Cały zakres 8A–8D ma dowody regresyjne i dopiero wtedy może trafić na produkcję |
...
Historyczny program A–7 pozostaje zamknięty. Nowy program zaczyna się od Fazy 8A, ponieważ przed kolejnymi zmianami trzeba usunąć ryzyko wycieku stanu logowania i odzyskać wiarygodny gate E2E. Fazy 8B, 8C i 8D mogą być później prowadzone niezależnie, ale żadna nie omija końcowej Fazy 9.
...
**Status: DONE.** Zakres `MOBILE-01–06` wdrożono i zweryfikowano unitami, lintem, buildem oraz izolowanym gate'em Playwright (53 passed, 22 oczekiwane skipy). Headed review potwierdził brak overflow przy 320/375/390 px i tekście 150%, stały dock nad polem przy wysokości 500 px, czytelny siedmioelementowy `BottomNav` oraz przejście rest timera `full → compact → full` bez zasłaniania inputu. Projekt i szczegółowa historia wykonania pozostają w Git.
...
**Status: DONE.** Zakres `FEEDBACK-01–04`, `NAV-01`, `MOBILE-07`, `A11Y-09–10`, `COPY-01–03`, `DEMO-01`, `TEST-04` został zaimplementowany, zweryfikowany i zintegrowany lokalnie z `puls-rebrand` przez fast-forward do `89452b7`. Zatwierdzony projekt i szczegółowa historia wykonania pozostają w Git. Faza nie otwiera ponownie Fazy 4 i nie obejmuje strategicznego redesignu marki.
...
**Status: DONE.** Zakres `AI-04`, `AI-05`, `AI-06`, `AI-12`, `AI-13` i `AI-14` został zaimplementowany, zweryfikowany i zintegrowany lokalnie z `puls-rebrand` przez fast-forward do `f0f5f7f`. Bramka objęła focused Vitest 51/51, pełny unit/support Vitest 59 plików i 467/467 testów, `npm run lint`, `npm run build` z 878 modułów oraz focused whole-branch review zakończony z Critical 0, Important 0 i Minor 0 po usunięciu pętli ponownej walidacji odrzuconego klucza. Bezpośrednia obserwacja Browser przy 390×844 potwierdziła konfigurację przed czatem i wyłączony composer z właściwą instrukcją. API klasyfikuje błędy Anthropic przez publiczne kody, waliduje plan przed zwróceniem go do UI i nie przepuszcza surowych detali upstreamu ani klucza do logów. README opisuje rzeczywisty limit `8/min` liczony transakcyjnie w Firestore. Push, deploy, publikacja indeksów i `RELEASE-08` nie zostały wykonane. Szczegóły i historia wykonania pozostają w Git.
...
**Status: DONE.** Bramy 7A i 7B zamknęły lokalny odbiór, a 7C zakończyła produkcyjne RELEASE-08–10. Stary `main` jest zachowany na `main-before-puls-2026-07-23`, Puls jest kanoniczny na `main` i wdrożony na `ironlog-coach.vercel.app`, wszystkie indeksy i restrykcyjne reguły Firestore są opublikowane, a post-rules smoke i pomiar zimnego dashboardu przeszły. Pełne dowody znajdują się w [`../audits/2026-07-23-phase-7c-production-release.md`](../audits/2026-07-23-phase-7c-production-release.md); szczegółowa historia wykonania pozostaje w Git.
...
**Cel:** przed kolejnymi zmianami usunąć artefakty, które nie powinny trafiać do repo ani produkcyjnego bundle'a, oraz przywrócić E2E jako wiarygodny sygnał po zmianach copy.

**Zakres kanoniczny:**

- **HYGIENE-01:** usunąć lokalny stan logowania Playwright z repozytorium i dodać trwałą regułę ignorowania; jeżeli token lub storage state opuścił lokalną maszynę, unieważnić powiązaną sesję.
- **HYGIENE-02:** usunąć albo przenieść poza `public/` robocze preview i screenshoty, które Vite publikuje razem z aplikacją. Materiały review mogą pozostać lokalnie lub w `docs/`, ale nie pod publicznym adresem produkcji.
- **TEST-GATE-01:** zaktualizować wspólne `expectAppReady` oraz bezpośrednie lokatory nagłówków po usunięciu końcowych kropek i zmianach mikrotekstu. Gotowość ekranu powinna opierać się na stabilnym kontrakcie strony, gdy dokładne copy nie jest przedmiotem testu.
- **TEST-GATE-02:** uruchomić dotknięte testy E2E na świeżych emulatorach, a następnie cały reprezentatywny zestaw tras desktop/mobile bez omijania błędów gotowości.

**Kryteria wyjścia:**

- `git status` i historia bieżącej gałęzi nie zawierają browser storage state, tokenów ani cookies;
- produkcyjny katalog statyczny nie zawiera roboczych preview ani screenshotów;
- wszystkie wspólne readiness checki rozpoznają aktualne ekrany;
- dotknięte testy E2E przechodzą bez retry, a ich porażka oznacza realny problem produktu lub jasno opisany problem harnessu.

**Poza zakresem:** przebudowa całego frameworka Playwright i tworzenie nowego systemu test IDs dla każdej kontrolki.

### Faza 8B — Serializacja projekcji workoutu

**Status: DESIGN IN PROGRESS.**

**Cel:** zagwarantować, że równoległe usunięcie i materializacja tego samego workoutu zawsze kończą się jednym autorytatywnym stanem, bez odtworzonych `exerciseSessions` i nieaktualnych rekordów.

**Zakres kanoniczny:**

- **WORKOUT-RACE-01:** ustalić jeden kontrakt serializacji operacji dla pojedynczego workoutu. Implementacja może użyć transakcyjnego stanu, tombstone'a albo innego istniejącego prymitywu Firebase, ale materializacja nie może pisać po rozpoczęciu skutecznego usunięcia.
- **WORKOUT-RACE-02:** usunięcie ma wyczyścić wszystkie projekcje workoutu i przeliczyć rekordy dla pełnej sumy dotkniętych kluczy ćwiczeń, niezależnie od tego, czy pośredni retry zdążył zmienić projekcję.
- **WORKOUT-RACE-03:** dodać deterministyczne testy integracyjne dla przeplotu `delete ↔ materialize/update`, w tym retry po przerwaniu każdej fazy operacji.

**Kryteria wyjścia:**

- po dowolnym testowanym przeplocie nie istnieją osierocone `exerciseSessions` dla usuniętego workoutu;
- rekordy użytkownika odpowiadają wyłącznie istniejącym workoutom;
- ponowienie obu operacji jest bezpieczne i prowadzi do tego samego stanu;
- UI nie pokazuje wpływu usuniętego treningu w historii ani postępach po zakończeniu synchronizacji.

**Decyzja wymagana przed implementacją:** wybrać mechanizm serializacji i zapisać kontrakt stanów oraz retry w osobnym planie Fazy 8B.

### Faza 8C — Integralność katalogu planów AI

**Status: DESIGN IN PROGRESS.**

**Cel:** nie dopuścić, aby walidacja planu AI po cichu przypisała ćwiczenie do złego źródła albo wygenerowała plan z niepełnego katalogu przedstawionego jako kompletny.

**Zakres kanoniczny:**

- **AI-CATALOG-01:** fallback po nazwie może zaakceptować ćwiczenie tylko wtedy, gdy nazwa jest jednoznaczna w połączonym katalogu globalnym i użytkownika. Duplikat nazwy ma prowadzić do odrzucenia lub ponownej generacji, nie do wyboru pierwszego wyniku.
- **AI-CATALOG-02:** błąd pobrania `userExercises` musi być jawnym stanem. Przed implementacją należy wybrać między retryable failure całej generacji a wyraźnie oznaczonym trybem ograniczonym do katalogu globalnego.
- **AI-CATALOG-03:** dodać testy dla identycznych nazw w obu źródłach, nieistniejących identyfikatorów, awarii katalogu użytkownika i zachowania klienta po zwróconym błędzie.

**Kryteria wyjścia:**

- każde ćwiczenie planu ma jednoznaczną parę `exerciseSource + exerciseId`;
- niejednoznaczna nazwa nigdy nie wybiera ćwiczenia na podstawie kolejności tablicy;
- użytkownik widzi prawdziwy stan generacji przy awarii własnego katalogu i ma działającą ścieżkę ponowienia;
- testy pokrywają kolizję nazw global/user i awarię odczytu Firestore.

**Decyzja wymagana przed implementacją:** zatwierdzić zachowanie produktu przy niedostępnym katalogu użytkownika.

### Faza 8D — Kontrakt repo i cleanup

**Status: READY.**

**Cel:** usunąć rozjazd między instrukcjami repo a aktualnym produktem oraz zamknąć potwierdzony martwy scaffolding bez refaktoru działających modułów.

**Zakres kanoniczny:**

- **DOC-01:** zaktualizować `AGENTS.md` tak, aby opisywał aktualny czerwono-recovery kierunek Puls, używane fonty i serwerową finalizację workoutu zamiast historycznego kontraktu klienta.
- **CLEANUP-01:** po sprawdzeniu importów usunąć nieużywane pliki startowe Vite i martwe assety wskazane przez `LATER-06`.

**Kryteria wyjścia:**

- instrukcje nie kierują kolejnych zmian do starego motywu ani nieaktualnego przepływu zapisu;
- `rg`, TypeScript i produkcyjny build potwierdzają brak referencji do usuniętych plików;
- cleanup nie zmienia zachowania ani wyglądu aplikacji.

**Poza zakresem:** porządkowanie działającego kodu wyłącznie dla estetyki i tworzenie nowych abstrakcji.

### Faza 9 — Korekcyjna bramka wydania

**Status: BLOCKED przez Fazy 8A–8D.**

**Cel:** zamknąć program korekcyjny jednym zestawem dowodów, zanim zmiany zostaną połączone z `main` lub wdrożone.

**Zakres kanoniczny:**

- **CORRECTIVE-RELEASE-01:** uruchomić lint, testy jednostkowe, build, testy reguł Firestore oraz integracje workoutu i AI.
- **CORRECTIVE-RELEASE-02:** uruchomić poprawiony zestaw E2E na świeżych emulatorach i ręczny smoke najważniejszych tras na desktopie oraz mobile.
- **CORRECTIVE-RELEASE-03:** wykonać failure injection dla wyścigu workoutu i awarii katalogu AI; same happy paths nie zamykają fazy.
- **CORRECTIVE-RELEASE-04:** potwierdzić brak tokenów, storage state i roboczych artefaktów w śledzonych plikach oraz produkcyjnym buildzie.
- **CORRECTIVE-RELEASE-05:** zapisać wyniki review i rollback. Push, merge i deploy wymagają osobnej, jawnej zgody użytkownika.

**Kryteria wyjścia:**

- wszystkie P1 z programu 8A–8D są zamknięte i mają test regresyjny;
- P2 są zamknięte albo jawnie przeniesione do LATER z zaakceptowanym uzasadnieniem;
- automatyczne bramki są zielone bez ukrywania realnych błędów przez retry;
- smoke nie pokazuje regresji logowania, dashboardu, planów, aktywnej sesji, historii, postępów ani AI;
- roadmapa, osobne plany faz i wynik końcowego review wskazują ten sam stan.
...
- **LATER-06:** przeniesione do aktywnej Fazy 8D jako `CLEANUP-01`; identyfikator pozostaje dla ciągłości śledzenia.
...
**Historyczny program A–7 i jego bramka produkcyjna są zakończone. Po Agent Sanity Review z 2026-07-29 aktywny jest nowy program korekcyjny.**

1. Zacząć od Fazy 8A, aby zabezpieczyć artefakty i naprawić sygnał E2E.
2. Przygotować i zatwierdzić osobne kontrakty dla Faz 8B oraz 8C. Obie fazy mogą być implementowane niezależnie po zamknięciu 8A.
3. Fazę 8D można wykonać równolegle po 8A; jest mała i nie wymaga zmiany zachowania produktu.
4. Nie łączyć ani nie wdrażać programu korekcyjnego przed przejściem Fazy 9.

Pierwszym szczegółowym planem powinien być plan Fazy 8A. Następnie należy rozstrzygnąć mechanizm serializacji workoutu w 8B; jest to najwyższe pozostałe ryzyko integralności danych.

## 9. Macierz śledzenia audytów

Ta sekcja jest obowiązkową warstwą kontroli kompletności. `ASR-1` oznacza techniczny Agent Sanity Review z brancha `puls-rebrand` na `8607eb6`, `ASR-UI` pierwszy audyt UI, `SDR` Senior Design Review z 2026-07-14, `REFINE-UI` pełny audyt runtime z 2026-07-20, a `ASR-2` Agent Sanity Review aktualnego repo z 2026-07-29. Jeden punkt audytu może zasilać kilka elementów roadmapy, ale żaden zweryfikowany problem nie może pozostać bez przypisania. Wartość `pełne` oznacza pełne odwzorowanie problemu w zakresie roadmapy — nie oznacza jeszcze, że problem został wdrożony lub zamknięty. Punkty odrzucone albo już zabezpieczone pozostają w macierzy, żeby nie wracały bez nowych dowodów.
...
| ASR-1-15 | Nieużywany scaffolding i assety Vite | P3 | CLEANUP-01, LATER-06 | pełne, przeniesione do aktywnej Fazy 8D po ponownym potwierdzeniu |
...
### ASR-2 — Agent Sanity Review, 2026-07-29

| ID audytu | Ustalenie | Priorytet audytu | Punkt roadmapy | Pokrycie |
|---|---|---:|---|---|
| ASR-2-01 | Usunięcie workoutu może przegrać wyścig z trwającą materializacją i pozostawić odtworzone projekcje | P1 | WORKOUT-RACE-01–WORKOUT-RACE-03 | pełne |
| ASR-2-02 | E2E używa nieaktualnych nagłówków po zmianach copy i zatrzymuje się w `expectAppReady` | P1 | TEST-GATE-01, TEST-GATE-02 | pełne |
| ASR-2-03 | Lokalny stan logowania Playwright nie ma trwałego wykluczenia z repo | P1 | HYGIENE-01 | pełne |
| ASR-2-04 | Fallback walidacji planu AI po nazwie może wybrać zły wpis przy duplikacie global/user | P2 | AI-CATALOG-01, AI-CATALOG-03 | pełne |
| ASR-2-05 | Awaria odczytu własnych ćwiczeń jest redukowana do pustej tablicy podczas generowania planu | P2 | AI-CATALOG-02, AI-CATALOG-03 | pełne |
| ASR-2-06 | Robocze preview i screenshot w `public/` są publikowane razem z produkcją | P2 | HYGIENE-02 | pełne |
| ASR-2-07 | `AGENTS.md` opisuje historyczny design system i nieaktualny kontrakt finalizacji workoutu | P2 | DOC-01 | pełne |
| ASR-2-08 | Potwierdzony nieużywany scaffolding i assety Vite pozostają w repo | P3 | CLEANUP-01, LATER-06 | pełne, przeniesione do aktywnej Fazy 8D |
