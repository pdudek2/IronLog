...
export function buildChatContextSections(context: AiUserContext) {
  ...
  const monthlyLine = [
    `${context.monthlyInsights.workoutCount} treningów / ${context.monthlyInsights.totalVolume} kg w ostatnich ${context.monthlyInsights.windowDays} dniach.`,
    `Średnio ${context.monthlyInsights.averageWorkoutVolume} kg na trening.`,
  ...
  return {
    profileLine: profileLine || 'Profil: brak danych.',
    readinessLine,
...
function buildMonthlyInsights({
  ...
  const weakBuckets = weeklyBuckets.filter((bucket) => {
  ...
  const lowReadinessStreak = findLowReadinessStreak(readinessEntries, since, now)
...
function findLowReadinessStreak(entries: AiReadinessInput[], since: number, now: number): AiReadinessInput[] {
  ...
  let current: AiReadinessInput[] = []

  for (const entry of sorted) {
    const score = computeReadinessScore(entry).score
    if (score < 55) {
      current.push(entry)
