...
test('ten exercise volumes retain readable chronology', async ({ page, cleanup }, info) => {
  ...
  await expect(page.getByText('Reps at record weight', { exact: true })).toBeVisible()
...
test('long template names and editor first action', async ({ page, cleanup }, info) => {
  const name = 'Powrót do regularnych workouts po dłuższej przerwie'
  ...
  const add = page.getByRole('button', { name: 'Add exercise', exact: true }).first()
  await add.focus()
  ...
  await add.click()
  await picker.getByPlaceholder('Search exercises...').fill('Squat')
  ...
  await page.getByPlaceholder('E.g. Upper / Lower 4 days').fill(name)
...
test('coach plan preview and goal validation', async ({ page }, info) => {
  await installMockAiRuntime(page, [{ kind: 'plan', plan: {
    name: 'Powrót do regularnych workouts', summary: 'Trzy spokojne days pracy nad siłą.',
  ...
  await page.getByRole('group', { name: 'AI Coach mode' }).getByRole('button', { name: /^Plan/ }).click()
  await page.getByRole('button', { name: 'Generate plan', exact: true }).click()
  await expect(page.getByRole('textbox', { name: 'Plan goal' })).toBeFocused()
  ...
  await page.getByRole('button', { name: 'Generate plan', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Powrót do regularnych workouts' })).toBeVisible()
  await expect(page.locator('[data-sonner-toast]')).toHaveCount(0, { timeout: 10000 })
  for (const width of [320, 393, 768]) {
    await page.setViewportSize({ width, height: 852 })
...
test('active ledger pending and completed controls', async ({ page, cleanup }, info) => {
  ...
  await page.getByRole('button', { name: 'Unmark set 1 exercise Phase 1 Bench Press', exact: true }).click()
  const pending = page.getByRole('button', { name: 'Mark set 1 exercise Phase 1 Bench Press', exact: true })
...
test('progress summary', async ({ page, cleanup }, info) => {
  ...
  await page.evaluate(() => document.fonts.ready)
  await expect(page.getByRole('group', { name: 'Sessions', exact: true })).toContainText('5')
  await expect(page.getByRole('group', { name: 'Volume', exact: true })).toContainText('+105%')
