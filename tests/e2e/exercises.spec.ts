...
test.describe('Exercises CRUD', () => {
  test('user exercise CRUD lifecycle is isolated', async ({ page, cleanup }) => {
    ...
    const addBtn = page.getByRole('button', { name: /Add custom/i })
      .or(page.getByRole('button', { name: /Nowe exercise/i }))
      .first()
    await expect(addBtn).toBeVisible({ timeout: 5_000 })
    await addBtn.click()

    const nameInput = page.getByPlaceholder('E.g. Banded Pull-apart')
    ...
    const editButton = page.getByRole('button', { name: `Edit exercise ${TEST_EXERCISE_NAME}` })
    ...
    await expect(page.getByRole('dialog', { name: 'Edit custom exercise' })).toHaveCount(0)
  ...
  test('global exercise detail page is reachable', async ({ page }) => {
    ...
    const globalSection = page.locator('section').filter({ hasText: 'Shared library' })
